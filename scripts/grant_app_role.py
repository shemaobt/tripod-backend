...
async def main() -> None:
    ...
    parser.add_argument("app_key", type=str, help="The unique key of the app (e.g., 'meaning-map-generator').")
    parser.add_argument("role_key", type=str, help="The role to grant (e.g., 'admin', 'annotator').")
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 2. Find app
        app = (await db.execute(select(App).where(App.app_key == args.app_key))).scalar_one_or_none()
        if not app:
        ...
        if existing:
            ...
            print(f"Updated existing access for {args.email} on {args.app_key} to role '{args.role_key}'.")
        else:
            new_role = UserAppRole(user_id=user.id, app_id=app.id, role_id=role.id)
            db.add(new_role)
            print(f"Granted new access for {args.email} on {args.app_key} with role '{args.role_key}'.")
